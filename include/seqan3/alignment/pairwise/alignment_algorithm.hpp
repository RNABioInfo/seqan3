   ...
   private:
    ...
    constexpr void make_alignment_result([[maybe_unused]] index_t const idx,
        ...
        if constexpr (traits_t::compute_end_positions) {
            ...
            if constexpr (traits_t::is_banded) {
                ...
                res.end_positions.first =
                    this->to_original_sequence1_position(res.end_positions.first);
                res.end_positions.second =
                    this->to_original_sequence2_position(res.end_positions.second);
            }
        }
        ...
        if constexpr (traits_t::compute_begin_positions) {
            ...
            res.begin_positions.first = this->to_original_sequence1_position(
                trace_res.first_sequence_slice_positions.first);
            res.begin_positions.second = this->to_original_sequence2_position(
                trace_res.second_sequence_slice_positions.first);
